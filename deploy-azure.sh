...
az functionapp config appsettings set -n $FUNCTION_APP_NAME -g $RESOURCE_GROUP --settings "TIMEOUT_SECONDS=$TIMEOUT_SECONDS" "CASSANDRA_HOST=$CASSANDRA_HOST" "CASSANDRA_PORT=$CASSANDRA_PORT" "CASSANDRA_USER=$CASSANDRA_USER" "CASSANDRA_PASSWD=$CASSANDRA_PASSWD" "CASSANDRA_KEYSPACE=$CASSANDRA_KEYSPACE" "KAFKA_BROKER=$KAFKA_BROKER" "KAFKA_BROKER_AUTH=$KAFKA_BROKER_AUTH" "KAFKA_TOPIC=$KAFKA_TOPIC" "KAFKA_CONSUMER_GROUP=$KAFKA_CONSUMER_GROUP" "KAFKA_SSL_ENABLED=$KAFKA_SSL_ENABLED" "KAFKA_USER=$KAFKA_USER" "KAFKA_PASSWORD=$KAFKA_PASSWORD"
if [ $? -ne 0 ] 
then
    return 1
fi
